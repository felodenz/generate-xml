...
def convert_json_to_esb(request):
    json_path = os.path.join(settings.MEDIA_ROOT, 'json_files\\test.json')
    ...
    #using api tag within path as nameof the api
    root.set("name", path)
    ...
    # Function to check if HTTP resquest connection exists
    def check_connections(connections):
        for connection_name, connection_details in connections.items():
            for main_connection in connection_details.get("main", []):
                for connection in main_connection:
                    node_name = connection.get("node", "")
                 #   if connection_name == "HTTP Request" and node_name.startswith("Respond to Webhook"):
                    if connection_name == "HTTP Request":
                        return True
        return False
    ...
    def find_url_matches(url):
        ...
        return {
            "query_match": (query_match, f"$ctx:query.param.{query_match.group(1)}" if query_match else None),
            "body_match": (body_match, f"json-eval($.{body_match.group(1)})" if body_match else None),
            "property_match": (property_match, f"$ctx:{property_match.group(1)}" if property_match else None),
            "header_match": (header_match, f"$trp:{header_match.group(1)}" if header_match else None),
            "route_path_params_match": (route_path_params_match, f"$ctx:uri.var.{route_path_params_match.group(1)}" if route_path_params_match else None)
        }
    ...
    def process_node(node_name, if_node_children, node_type_lookup, extracted_data, in_sequence, level=0):
       ...
       # pattern = r'={{ \$json\.query\.(.*?) }}'
        pattern = r'query\.(\w+)'
        assing_value = target_node["parameters"]["conditions"]["conditions"][0]["leftValue"]
        ...
        print("chadore", remove_equals_sign(assing_value))
        # if query_match:
    ...
    def find_and_process_filter_sequences(nodes):
        ...
        for outer_if_node in outer_if_nodes:
            ...
            if result == True:
                ...
                process_node_outSeq(outer_if_node, if_node_children, node_type_lookup, extracted_data, in_sequence)
            else:
    ...
    def create_payload_factory_inSeq(response_body):
        ...
        # Add the <arg> elements dynamically using the extracted values
        for value in arg_vars:
            cleaned_value = remove_equals_sign(value)  # Remove equals sign if present
    ...
    for node_name in node_order:
        ...
        if node:
            ...
            exists = check_connections(extracted_data["connections"])
    ...
    # output_file = "output_esb.xml"
    # with open(output_file, "w") as f:
    #     f.write(prettify(root))
    
    # print(f"ESB XML configuration saved to {output_file}")

    # print("chips", prettify(root))
    # pretty_xml = parseString(prettify(root)).toprettyxml(indent="  ")
